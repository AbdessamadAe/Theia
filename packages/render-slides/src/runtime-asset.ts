...
/**
 * Bundle @theia/runtime's browser entry into a single minified IIFE string,
 * ready to inline into the deck. We bundle the package's TypeScript *source*
 * directly with esbuild (no separate build step needed), so `theia build` and
...
export function inlinedRuntimeJs(): string {
  ...
  const pkgJson = require.resolve("@theia/runtime/package.json");
  ...
  if (!out) throw new Error("Failed to bundle @theia/runtime browser entry");
