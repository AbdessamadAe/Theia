...
import { BoardIcon, EASEL_MOUNT, EaselFrame, EmptyBoardSketch } from "@/components/theia-art";
 ...
 *  - "easel": the same live preview MOUNTED on a static theiaboard-easel
 *    illustration.
...
export function PreviewFrame({
  ...
  const [easelOn, setEaselOn] = React.useState<boolean>(() => {
    try {
      return localStorage.getItem("theia-easel") === "on"; // default OFF
    } catch {
      return false;
    }
  });
  ...
  const toggleEasel = (): void =>
    setEaselOn((v) => {
      const next = !v;
      try {
        localStorage.setItem("theia-easel", next ? "on" : "off");
      } catch {
        /* storage unavailable */
      }
      return next;
    ...
    html && slides > 0 ? (
      ...
      <div className="text-muted-foreground flex h-full flex-col items-center justify-center gap-4 p-6 text-center">
        ...
        <div className="space-y-1">
          <p className="theia-display text-foreground text-lg font-medium">Nothing on the board yet</p>
  ...
  const controls = (
    <div
      className="pointer-events-none absolute inset-x-0 bottom-3 z-10 flex items-center justify-center opacity-0 transition-opacity duration-base ease-theia group-hover/preview:opacity-100 focus-within:opacity-100"
  ...
  const toggle = wide ? (
    <Hint label={easelOn ? "Use the plain preview" : "Frame on a theiaboard easel"}>
      <button
        type="button"
        aria-pressed={easelOn}
        aria-label="Toggle theiaboard easel framing"
        onClick={toggleEasel}
