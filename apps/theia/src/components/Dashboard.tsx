...
import { BoardIcon as TheiaBoardIcon, EmptyBoardSketch, WordmarkFlourish } from "@/components/theia-art";
import {
  DownloadIcon,
  MoonIcon,
  MoreIcon,
...
export function Dashboard({ theme, setTheme, onOpen }: DashboardProps): React.ReactElement {
  ...
  const [files, setFiles] = React.useState<Map<string, TheiaFile[]>>(new Map());
  ...
  const onExportTheia = async (p: Project): Promise<void> => {
    ...
    >
      ...
      <header className="bg-card/60 sticky top-0 z-10 flex items-center gap-3 border-b px-4 py-3 backdrop-blur sm:px-6">
        ...
        <Segmented
          ...
          className="p-0.5"
          options={[
            { value: "theiaboard", label: <TheiaBoardIcon className="size-4" />, ariaLabel: "Theia theme", title: "Theia" },
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6">
        ...
        {loading ? null : shown.length === 0 ? (
          <div className="flex flex-col items-center gap-4 py-16 text-center">
            ...
            <div>
              <p className="theia-display text-foreground text-xl font-medium">
            ...
            {shown.map((p) => (
              ...
              >
                ...
                <ProjectMenu
                  ...
                  onExportTheia={() => void onExportTheia(p)}
      ...
      {dragging && (
        <div className="bg-background/70 border-live pointer-events-none fixed inset-0 z-40 m-4 flex items-center justify-center rounded-2xl border-2 border-dashed backdrop-blur-sm">
          <p className="theia-display text-foreground text-2xl">Drop to import</p>
        </div>
      )}
    ...
    <div ref={ref} className="absolute right-3 top-3">
      ...
      {open && (
        <div role="menu" className="bg-popover absolute right-0 z-20 mt-1 w-44 rounded-lg border p-1 shadow-3">
          ...
          <button role="menuitem" className={item} onClick={() => { setOpen(false); props.onExportProject(); }}>Export project (.theiaproj.json)</button>
          <button role="menuitem" className={item} onClick={() => { setOpen(false); props.onExportTheia(); }}>Export .theia</button>
