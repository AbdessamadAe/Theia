...
const channel =
  typeof BroadcastChannel !== "undefined" ? new BroadcastChannel("theia-projects") : null;
...
export async function getFilesByProject(projectId: string): Promise<TheiaFile[]> {
  const files = await withTx(["files"], "readonly", (tx) =>
    reqToPromise(
      tx.objectStore("files").index("byProject").getAll(projectId) as IDBRequest<TheiaFile[]>,
    ),
  );
...
export async function getPrimaryFile(projectId: string): Promise<TheiaFile | undefined> {
  return (await getFilesByProject(projectId))[0];
}

/** All files, grouped by project — one query to feed dashboard thumbnails. */
export async function getFilesGrouped(): Promise<Map<string, TheiaFile[]>> {
  const files = await withTx(["files"], "readonly", (tx) =>
    reqToPromise(tx.objectStore("files").getAll() as IDBRequest<TheiaFile[]>),
  );
  const map = new Map<string, TheiaFile[]>();
...
export async function updateFileSource(fileId: string, source: string): Promise<number> {
  ...
  const projectId = await withTx(["files", "projects"], "readwrite", async (tx) => {
    ...
    const file = await reqToPromise(files.get(fileId) as IDBRequest<TheiaFile | undefined>);
