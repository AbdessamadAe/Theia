...
export function DocCode({ code, lang = "chalk" }: { code: string; lang?: "chalk" | "bash" }): React.ReactElement {
  ...
  return (
    <div className="bg-card relative my-4 overflow-hidden rounded-lg border shadow-1">
      <pre className="overflow-x-auto px-4 py-3 pr-12 font-mono text-[13px] leading-relaxed">
        <code>{lang === "chalk" ? highlightChalk(code) : code}</code>
      </pre>
      <button
        type="button"
        onClick={() => void copy()}
    ...
    <div className="my-4">
      ...
      <div className="-mt-2 mb-2 flex items-center gap-3">
        {live && (
          <button
            type="button"
            onClick={() => setHtml(compileChalk(source, { assets: ASSETS }).html)}
